...
for loop=12:13
    ...
    elseif (loop == 12)
        ...
        Dest = 'C:\Users\John Mauceri\DICOM_TEST\DICOM_rect\Location3\xyradius_auto_Noha_CLIP_Benign.txt';
    elseif (loop == 13)
        CircleData = 'C:\Users\John Mauceri\Desktop\Noha\US and CLIP Images of Cancer Cases\';
        Dest = 'C:\Users\John Mauceri\DICOM_TEST\DICOM_rect\Location3\xyradius_auto_Noha_CLIP_Cancer.txt';
    end
    ...
    for i = 3:length(files)
        if ((loop == 12) || (loop ==13))
            clip = dir(strcat(CircleData, files(i).name, '\CLIP*'));
            if (size(clip,1) == 0) continue; end
            clip.name = strcat(clip.name, '\');
        else
            clip.name = '';
        end
        list_files = dir(strcat(CircleData, files(i).name, '\', clip.name, '*.jpg'));
        for j = 1:length(list_files)
            i
            strcat(CircleData, files(i).name, '\', clip.name, list_files(j).name)
            if (list_files(j).name(1) == '.') continue; end;
            I = imread(strcat(CircleData, files(i).name, '\', clip.name, list_files(j).name));
