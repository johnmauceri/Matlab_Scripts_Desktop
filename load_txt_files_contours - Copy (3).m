...
for i = 1:2
    ...
    for loop = filename'
        ...
        VC = floor(D(:, 3));
        for lp = 1:size(XC)
            I_copy(YC_flip(lp),XC(lp)) = 1;%TEST MODE- change 1 4000
        end;
        ...
        rect = int64(rect);
        I = imcrop(I, rect);
        I_copy = imcrop(I_copy, rect);
        %figure;
        %imagesc(I);
        %colormap gray;
        A = im2col(I, [SAMPLE_SQUARE SAMPLE_SQUARE], 'distinct');
        A_copy = im2col(I_copy, [SAMPLE_SQUARE SAMPLE_SQUARE], 'distinct');
        sz= size(A);
        ...
        for n = 1: sz(2)
            ...
            B_copy = reshape(A_copy(:,n), SAMPLE_SQUARE, SAMPLE_SQUARE);
            %figure;%TEST MODE- UNmake comment
            %imagesc(B_copy);%TEST MODE- UNmake comment
            %colormap gray;%TEST MODE- UNmake comment ADD BREAKPOINT!!!!!
            if i==1
                ...
                fnc = fullfile(dirDest, strcat('Can',  int2str(cnt), '_', int2str(n), '_', desc, '_', tmp{1}, '_Q', num2str(Q), '_T', num2str(type), '_X', num2str(rect(1)+xinc), '_', 'Y', num2str(rect(2)+yinc), '_', num2str(SAMPLE_SQUARE), 'X', num2str(SAMPLE_SQUARE), '_', num2str(INFO.PixelSpacing(1)), 'X', num2str(INFO.PixelSpacing(2)), '_CNTR', '.jpg'));
            else
                ...
                fnc = fullfile(dirDest, strcat('Ben',  int2str(cnt), '_', int2str(n), '_', desc, '_', tmp{1}, '_Q', num2str(Q), '_T', num2str(type), '_X', num2str(rect(1)+xinc), '_', 'Y', num2str(rect(2)+yinc), '_', num2str(SAMPLE_SQUARE), 'X', num2str(SAMPLE_SQUARE), '_', num2str(INFO.PixelSpacing(1)), 'X', num2str(INFO.PixelSpacing(2)), '_CNTR', '.jpg'));
            end
            if ((nnz(B)/prod(size(B)) > PERCENT_BLACK) | (n == 1))
                imwrite(mat2gray(B), fn, 'jpg'); %TEST MODE- make comment
                imwrite(mat2gray(B_copy), fnc, 'jpg'); %TEST MODE- make comment
            end
