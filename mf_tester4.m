...
for i = 1:m
    ...
    if (isempty(strfind(file, 'M00016'))); continue; end;
    df = dir(file);
    ...
    for j = 1:4
        ...
        if (~isempty(strfind(file, 'CC')))
            ...
            if ((found_CC == 1) && (found_MLO == 1))
                ...
                fprintf(line_wedge_endpts,'%s   %4.0f %4.0f    %4.0f %4.0f    %4.0f %4.0f    %4.0f %4.0f    %4.0f %4.0f    %4.0f %4.0f\n', file(45:83), X, height-Ymin, X, height-Ymax, X-demin, height-Ymin, X+demin, height-Ymax, X-demax, height-Ymin, X+demax, height-Ymax);
            end
        end
