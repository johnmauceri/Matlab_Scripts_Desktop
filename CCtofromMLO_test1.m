...
% FIX cnt XCC YCC XMLO YMLO ps,height, width for both
        ...
        width = INFO.Width;
            ...
            if (~isempty(strfind(file, 'MLO')))
                ...
                if (j == 2)
                    if (~isempty(strfind(file, '_L_'))) X(i) = double(width) - X(i); end;
            ...
            if (~isempty(strfind(file, 'CC')))
                ...
                if (j == 1)                  
                    if (~isempty(strfind(file, '_L_'))) XCC(i) = double(width) - XCC(i); end;
                    if (INFO.PatientOrientation(1) == 'A') XCC(i) = double(width) - XCC(i); end;
                    plot(XCC(i), XCC(i), 'X');
                    dist = XCC(i) - x_nip;
                    
                    fn_box = fullfile(dirDest, strcat(file(45:83), '_BOX.jpg'));
                    IC = I;
                    YCC(i) = double(height) - YCC(i);
                    IC([(YCC(i)-100) (YCC(i)-99) (YCC(i)+99) (YCC(i)+100)],(XCC(i)-100):(XCC(i)+100)) = 4000;
                    IC((YCC(i)-100):(YCC(i)+100),[(XCC(i)-100) (XCC(i)-99) (XCC(i)+99) (XCC(i)+100)]) = 4000;
                    imwrite(mat2gray(IC), fn_box, 'jpg');
                end
                ...
                if (j == 2)
                    %Go from MLO to CC
                    XCC(i) = x_nip + dist;
                    Ymax = max(Iby(find(Ibx==XCC(i))));
                    Ymin = min(Iby(find(Ibx==XCC(i))));
                    line([XCC(i) XCC(i)],[Ymax Ymin]);

                    IC = I;
                    IC([(height-Ymax):(height-Ymin)], [XCC(i)-1:XCC(i)+1]) = 4000;
                    fn_line = fullfile(dirDest, strcat(file(45:83), '_LINE.jpg'));
                    imwrite(mat2gray(IC), fn_line, 'jpg');
                end
            end                       
