...
if (((cleav_x(2) - cleav_x(1)) < 100) && ((cleav_x(2) - cleav_x(1)) > 0)) cleav_x(1) = cleav_x(2) - 100; end
...
if (length(lines) < 5) lines = houghlines(BW,T,R,P,'FillGap',5,'MinLength',200); end
if (length(lines) < 5) lines = houghlines(BW,T,R,P,'FillGap',5,'MinLength',100); end
...
for k = 1:length(lines)
    ...
    if ((angl > MIN_MUSCLE_ANGLE) && (angl < MAX_MUSCLE_ANGLE))
        ...
        x_boarder = find(Iboundary(int16(xy(2,2)),:), 1, 'last');
        if (isempty(x_boarder)) x_boarder = 1; end
        dist2boarder = double(xy(2,1) - x_boarder);
        if (((dist2boarder / dist2edge) > EDGE_BOARDER_RATIO) || (xy(1,2) < 500))
            if ((cleav_y(2) - xy(2,2)) > (0.25*cleav_y(2)))
                ...
                %cr = uint16(1000*mean2(double(I(xy(1,2):topy,botx:xy(2,1)))) / mean2(double(I(boty:xy(2,2),xy(1,1):topx))));
                if (cr > max_cr)
                    max_cr = cr;
                    len_of_cr = len;
                    xy_cr = xy;
...
%line with the highest contrast ratio provided it's CR isn't too low too.
if ((cr_of_len < 1500) && (max_cr > 1650)) xy_long = xy_cr; max_len = len_of_cr; cr_of_len = max_cr; end
if (cr_of_len == 0)  xy_long = 0; max_len = 0; end
...
if (DEBUG>=1)
    ...
    if (xy_long ~= 0)
        if (cr_of_len < 1500)
            saveas(h, strcat('H:\birad_fig\', file, '_CR',num2str(cr_of_len),'.png'), 'png');
        end
    else
