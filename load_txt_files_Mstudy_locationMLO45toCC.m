...
dirDest = 'C:\Users\John Mauceri\DICOM_TEST\DICOM_rect\Location45';
...
for i = 1: m
    i
    %if (isempty(strfind(fn{i}, 'M00015'))); continue; end;
    ...
    for file = files'
        ...
        if ((isempty(strfind(file.name, '_L_')) && rl) || (~isempty(strfind(file.name, '_L_')) && ~rl))
            ...
            if (~isempty(strfind(file.name, 'MLO')))
                ...
                y_spaceM(6) = ymax; y_spaceM(5) = ymax - ((ymax - y_nip)/2.5); y_spaceM(4) = ymax - 2*((ymax - y_nip)/2.5);
                y_spaceM(1) = ymin; y_spaceM(2) = ymin + ((y_nip - ymin)/2.5); y_spaceM(3) = ymin + 2*((y_nip - ymin)/2.5);
            
                figure; hold on; plot(Ibx, Iby, '.', x_nip, y_nip, 'o', xmax, y_spaceM, '.');
                ...
                if (rl)
                    %right
                    if ((location(i) == 10) || (location(i) == 11)) ybase = y_spaceM(6); end;
                    if ((location(i) ==  9) || (location(i) == 12)) ybase = y_spaceM(5); end;
                    if ((location(i) ==  1) || (location(i) ==  8)) ybase = y_spaceM(4); end;
                    if ((location(i) ==  2) || (location(i) ==  7)) ybase = y_spaceM(3); end;
                    if ((location(i) ==  3) || (location(i) ==  6)) ybase = y_spaceM(2); end;
                    if ((location(i) ==  4) || (location(i) ==  5)) ybase = y_spaceM(1); end;
                else
                    %left
                    if ((location(i) ==  1) || (location(i) ==  2)) ybase = y_spaceM(6); end;
                    if ((location(i) ==  3) || (location(i) == 12)) ybase = y_spaceM(5); end;
                    if ((location(i) ==  4) || (location(i) == 11)) ybase = y_spaceM(4); end;
                    if ((location(i) ==  5) || (location(i) == 10)) ybase = y_spaceM(3); end;
                    if ((location(i) ==  6) || (location(i) ==  9)) ybase = y_spaceM(2); end;
                    if ((location(i) ==  7) || (location(i) ==  8)) ybase = y_spaceM(1); end;
                end
