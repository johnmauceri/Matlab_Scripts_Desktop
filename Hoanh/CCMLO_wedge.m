function [lin, err] = CC2MLO_wedge(filenameCC, filenameMLO, Direction, Xin, Yin)
%{
...
OUTPUT lin      x1 y1 (Line)
                ...
                x2 y2 (Max Wedge)
%}
DEBUG = 0; %0 - none, 1 - detailed, 2 - plots and saved images
dirDest = pwd;
...
for j = (1+2*Direction):(2+2*Direction)
    ...
    I = dicomread(file);
    INFO = dicominfo(file);
    char(INFO.PatientOrientation);
    size_match = size(strmatch('A',char(INFO.PatientOrientation)));
    if size_match(1)~=0
    	I      = flip(I,2);
    end
