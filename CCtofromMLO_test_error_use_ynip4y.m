...
dirDest = 'C:\Users\John Mauceri\DICOM_TEST\DICOM_rect\Location4';
...
XCC = zeros(1, m)'; YCC = zeros(1, m)'; XMLO = zeros(1, m)'; YMLO = zeros(1, m)';
    ...
    for pass = 1:2
        ...
        for j = 1:4
            ...
            if (~isempty(strfind(file, 'MLO')))
                ...
                if (pass == 2) figure; hold on; plot(Ibx, Iby, '.', x_nip, y_nip, 'o'); end
                ...
                if ((found_CC == 0) && (found_MLO == 1))
                    ...
                    X = uint16(x_nip + 2 * 10 / ps(1)); %2cm from nipple
                    Y = uint16(y_nip + 2 * 10 / ps(2)); %2cm from nipple
                    if (pass == 2) plot(X, Y, 'X'); end
                    dist = X - x_nip;
                    ...
                    %Rotate point specified the same angle
                    ytmp = double(height) - uint16(y_nip);
                    X = uint16(x_nip + 2 * 10 / ps(1)); %2cm from nipple
                    BW3=BW;
                    BW3(:,:)=0;
                    BW3(ytmp-1:ytmp+1, X-1:X+1)=1;
                ...
                if ((found_CC == 1) && (found_MLO == 1))
                    ...
                    if (pass == 2) line([X X],[Ymax Ymin]); end
                    ...
                    if ((Xmax - Xmin) > 10)
                        for step = Xmin: Xmax
                            ...
                            if (pdist([Xtmp (double(height)-YMLO(i)); step out]) < min_dist)
                                min_dist = pdist([Xtmp (double(height)-YMLO(i)); step out]);
                                Xstore = step;
                            end
                        end
                    else
            ...
            if (~isempty(strfind(file, 'CC')))
                ...
                if ((found_CC == 1) && (found_MLO == 0))
                    ...
                    Y = double(height) - Y;
                    XCC(i) = X;
                    YCC(i) = double(height) - uint16(y_nip);
                    IC([(Y-100) (Y-99) (Y+99) (Y+100)],(X-100):(X+100)) = 4000;
                    IC((Y-100):(Y+100),[(X-100) (X-99) (X+99) (X+100)]) = 4000;
                    imwrite(mat2gray(IC), fn_box, 'jpg');
                end
            ...
            if (pass == 2) title(strrep(file(45:84), '_', ' ')); end
        end
    end
end
fclose(fn_error);
