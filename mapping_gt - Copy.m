SourceGT_0 = 'C:\Users\John Mauceri\DICOM_TEST\DICOM_rect\Location3\xyradius_auto_John_dummy_GOLDEN.txt';  %JGMM remove line
...
for i = 0:0 % loop over all of Noha's files                        JGMM 0:10
    if i == 0 SourceGT = SourceGT_0; CircleData = CircleData0; end  %JGMM remove line
    %if i == 0 SourceGT = SourceGT0; CircleData = CircleData0; end   %JGMM put back
    ...
    for loop = 1: size(accession,1)   %loop for every one of Noha's circles (multiple per image)
        ...
        if (~isempty(strfind(file, 'MLO')))
            ...
            vert_pct = ((double(height) - ytmp(1)) - ymin) / (ymidmax - ymidmin);
        ...
        if (~isempty(strfind(file, 'CC')))
            ...
            [ymidmin ymidmax] = calc_yminmax(Ibx, Iby, y_nip, height, Xmid);
            vert_pct = ((double(height) - Y) - ymin) / (ymidmax - ymidmin);
