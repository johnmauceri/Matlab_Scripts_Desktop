...
fn_camera_angle = fopen(fullfile('C:\Users\John Mauceri\DICOM_TEST\DICOM_LOCATION\Mstudy', 'hist_camera_angle.txt'),'wt');
fprintf(fn_camera_angle,'Filename                                 Camera Angle\n');
fn_muscle_angle = fopen(fullfile('C:\Users\John Mauceri\DICOM_TEST\DICOM_LOCATION\Mstudy', 'hist_muscle_angle.txt'),'wt');
fprintf(fn_muscle_angle,'Filename                                 Muscle Angle\n');
...
for i = 1: m
    i
    %if (isempty(strfind(fn{i}, 'M00116'))); continue; end;
    ...
    for file = files'
        ...
        if ((isempty(strfind(file.name, '_L_')) && rl) || (~isempty(strfind(file.name, '_L_')) && ~rl))
            ...
            if (~isempty(strfind(file.name, 'MLO')))
                ...
                fprintf(fn_camera_angle,'%s %5.1f\n', file.name(1:40), INFO.PositionerPrimaryAngle);
                fprintf(fn_muscle_angle,'%s %5.1f\n', file.name(1:40), atan((xy_long(1,2)-xy_long(2,2))/(xy_long(1,1)-xy_long(2,1)))*180/pi);
            end
...
fclose(fn_camera_angle);
fclose(fn_muscle_angle);