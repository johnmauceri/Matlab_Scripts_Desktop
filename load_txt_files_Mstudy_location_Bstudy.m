Source = 'C:\Users\John Mauceri\DICOM_TEST\DICOM_LOCATION\Bstudy\homa_2014_biopsies_no_mrns-homa_2014_biopsies_no_mrns-includes-all-den4-since2010.txt'; 

SourceData = 'C:\Users\John Mauceri\Desktop\Bstudy\';

dirDest = 'C:\Users\John Mauceri\DICOM_TEST\DICOM_rect\Bstudy';
...
for i = 1:m
    ...
    %if (isempty(strfind(fn, '00142'))); continue; end;
    ...
    for file = files'
        ...
        if ((isempty(strfind(file.name(1:39), '_L_')) && rl) || (~isempty(strfind(file.name(1:39), '_L_')) && ~rl))
            ...
            if (~isempty(strfind(file.name, '_MLO_')))
                ...
                if (DEBUG>0) title(strrep(file.name(1:39), '_', ' ')); end
            ...
            if (~isempty(strfind(file.name, '_CC_')))
                ...
                fn_box = fullfile(dirDest, strcat(file.name(1:38), '_BOX.jpg'));
            end                       
            title(strrep(file.name(1:39), '_', ' '));
            
            IC = I;
            %Next 3 lines insert bounding box to matrix and save file
            xsize = int16(10 * siz(i) / (2 * ps(1)));
            ysize = int16(10 * siz(i) / (2 * ps(2)));
            IC=false(size(I));
            IC([(y_cancer-ysize) (y_cancer-ysize-1) (y_cancer+ysize-1) (y_cancer+ysize)],(x_cancer-xsize):(x_cancer+xsize)) = true;
            IC((y_cancer-ysize):(y_cancer+ysize),[(x_cancer-xsize) (x_cancer-xsize-1) (x_cancer+xsize-1) (x_cancer+xsize)]) = true;
            overlayImage = imoverlay(mat2gray(I), IC(1:size(I,1),1:size(I,2)), [1,0,0]);
            imwrite(overlayImage, fn_box, 'jpg');
            fprintf(fn_location,'%s  %4.0f %4.0f %6.4f\n', file.name, x_cancer, y_cancer, siz(i));
        end
    end
end 
fclose(fn_location);