...
for ang = 180;%0:30:330
    ...
    if (DEBUG>=1) figure(3); imagesc(Ismall);colormap gray; end;
    
    step = size(filt_rot,1) - 1;
    J = zeros(size(Ismall));
    for y = 1:5:size(Ismall,1)-step %421
        for x = 1:5:size(Ismall,2)-step %526
            box = Ismall(y:y+step,x:x+step);
            mn = mean2(box);
            stnd = std2(box);
            box = double(box > (mn+(1*stnd)));
            figure(9); imagesc(box);colormap gray;
            BW =  filt_rot .* box;
            BW1 = box .* box;
            bw1 = sum(BW1(:));                        
            if (bw1 == 0)
...
max(max(J))
[iy, ix] = find(J == max(max(J)));
if (DEBUG>=1) figure(10); imagesc(I);colormap gray; hold on; end;
