...
for i = 1:m
    ...
    for j = 1:4
        ...
        if (~isempty(strfind(file, 'MLO')))
            ...
            BW = BW1;
            BW=imresize(BW,4);
            %if (j == 2) figure; imagesc(BW);colormap gray; end
            ...
            BW = (BW==idx);
            %Create boundary to remove edge from filter
            BW1 = (imdilate(double(BW==1),ones(100)));
            nnz_bw1 = nnz(BW1);
            %Create boundary to get original muscle
            BW = (imdilate(double(BW==1),ones(200)));
            %Get original muscle
            BW = I .* uint16(BW);
            %Use small filter to better define muscle
            BW=imfilter(double(BW), filt3, 'replicate');
            %Remove bad edges from filter
            BW = BW .* BW1;
            %Normalize
            BW = (BW - min(min(BW)))/(max(max(BW)) - min(min(BW)));
            %Threshold
            ...
            while (mt < 1.0)
                BW1 = BW > mt;
                if (nnz(BW1) < (nnz_bw1 * PERCENT_WHITE2)) break; end;
                mt = mt + 0.05;
            end
            BW = BW1;
            %}
            ...
            %if (j == 2) figure, imagesc(I), colormap gray; end
            if (j == 2) figure, imagesc(I), colormap gray, hold on; end
            max_len = 0;
            min_angl = 90;
            max_angl = 0;
            for k = 1:length(lines)
                ...
                %JGMMif (DEBUG==1) plot(xy(1,1),xy(1,2),'x','LineWidth',2,'Color','yellow'); end;
                %JGMMif (DEBUG==1) plot(xy(2,1),xy(2,2),'x','LineWidth',2,'Color','red'); end;
                ...
                if ((angl > MIN_MUSCLE_ANGLE) && (angl < MAX_MUSCLE_ANGLE))
                    ...
                    if ((dist2boarder / dist2edge) > EDGE_BOARDER_RATIO)
                        %if (j == 2) plot(xy(:,1),xy(:,2),'LineWidth',2,'Color','cyan'); end
                        if (len > max_len)
                            max_len = len;
                            xy_long = xy;
                        end
            ...
            if (j == 2) if (max_len > 0)   plot(xy_long(:,1),xy_long(:,2),'LineWidth',2,'Color','blue'); end; end
            if (j == 2) if (max_angl > 0)  plot(xy_long_max(:,1),xy_long_max(:,2),'LineWidth',2,'Color','yellow'); end; end
            if (j == 2) if (min_angl < 90) plot(xy_long_min(:,1),xy_long_min(:,2),'LineWidth',2,'Color','red'); end; end
            %END ADJUSTED MATLAB CODE FOR USING HOUGHLINES
            if (j == 2) title(strrep(file(45:85), '_', ' ')); end
