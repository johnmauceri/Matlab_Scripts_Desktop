    ...
    for j = 1:4
        ...
        if (~isempty(strfind(file, 'MLO')))
            ...
            if ((found_CC == 1) && (found_MLO == 1))
                ...
                if (Ymax < y_nip) Ymax = ymax; end
                if (Ymin > y_nip) Ymin = ymin; end
                line([X X],[Ymax Ymin]);
                ...
                fprintf(fn_error,'%s %.2f\n', file(45:84), sign(Xtmp - Xstore) * min_dist * ps(1) / 10);
                sign(Xtmp - Xstore) * min_dist * ps(1) / 10 %Note assumes ps(1) & ps(2) the same calc min_dist in cm to fix
        ...
        if (~isempty(strfind(file, 'CC')))
            ...
            if ((found_CC == 1) && (found_MLO == 1))
                ...
                if (~isempty(strfind(file, '_L_'))) 
                    Xtmp = double(width) - XCC(i);
                else
                    Xtmp = XCC(i);
                end;
                if (INFO.PatientOrientation(1) == 'A') Xtmp = double(width) - Xtmp; end;
                fprintf(fn_error,'%s %.2f\n', file(45:83), (Xtmp - X) * ps(1) / 10);
            end
        end
