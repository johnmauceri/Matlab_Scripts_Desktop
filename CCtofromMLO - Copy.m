...
for i = 1: m
    i
    %if (isempty(strfind(fn{i}, 'M00076'))); continue; end;
    ...
    for file = files'
        ...
        if ((isempty(strfind(file.name, '_L_')) && rl) || (~isempty(strfind(file.name, '_L_')) && ~rl))
            ...
            if (~isempty(strfind(file.name, 'MLO')))
                ...
                line([x_nip xmax],[y_nip y_space(4)]);
                ...
                if ((found_CC == 0) && (found_MLO == 1))
                    plot(X(i), Y(i), 'X');
                    %dist = ((ps(1) * (X(i) - x_nip) / 10)^2 +  (ps(2) * (Y(i) - y_nip) / 10)^2)^0.5;
                    dist = X(i) - x_nip;
                    %theta = atan((Y(i) - y_nip) / (X(i) - x_nip));
                end
                if ((found_CC == 1) && (found_MLO == 1))
                    %Go from CC to MLO
                    %X(i) = x_nip + (10 * dist * cos(theta) / ps(1));
                    %Y(i) = y_nip + (10 * dist * sin(theta) / ps(2));
                    X(i) = x_nip + dist;
                    Y(i) = y_nip;
                    plot(X(i), Y(i), 'X');
                end
            ...
            %IC([(y_cancer-100) (y_cancer-99) (y_cancer+99) (y_cancer+100)],(x_cancer-100):(x_cancer+100)) = 0;
            %IC((y_cancer-100):(y_cancer+100),[(x_cancer-100) (x_cancer-99) (x_cancer+99) (x_cancer+100)]) = 0;
            Y(i) = double(height) - Y(i);
            IC([(Y(i)-100) (Y(i)-99) (Y(i)+99) (Y(i)+100)],(X(i)-100):(X(i)+100)) = 0;
            IC((Y(i)-100):(Y(i)+100),[(X(i)-100) (X(i)-99) (X(i)+99) (X(i)+100)]) = 0;
            imwrite(mat2gray(IC), fn_box, 'jpg');
        end
    end
    %FINISHED WITH ONE CC AND ONE MLO
end 