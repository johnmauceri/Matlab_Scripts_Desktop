...
filt4 = [ones(5,10);-1*ones(5,10)];
filt5 = imrotate(filt4,FILTER_ANGLE);
%Make filter sum to 1
filt6 = filt5 / (sum(sum(filt5)));
...
for i = 1:m
    ...
    for j = 1:4
        ...
        if (~isempty(strfind(file, 'MLO')))
            ...
            BW = (BW==idx);
            if (j == 2) figure; imagesc(BW);colormap gray; end
            BW = (imdilate(double(BW==1),ones(50)));
            if (j == 2) figure; imagesc(BW);colormap gray; end
            
            %BW2 = bwboundaries(BW);
            %save = BW2{1};
            %BW3 = zeros(size(BW,1), size(BW,2));
            %for ii = 1:size(save,1)
            %    BW3(save(ii,1), save(ii,2)) = 1;
            %end
            %if (j == 2) figure; imagesc(BW3);colormap gray; end
            
            BW = I .* uint16(BW);
            if (j == 2) figure; imagesc(BW);colormap gray; end
            BW=imfilter(BW, filt6, 'replicate');
            if (j == 2) figure; imagesc(BW);colormap gray; end
            BW(BW > 43000) = 0;
            if (j == 2) figure; imagesc(BW);colormap gray; end
