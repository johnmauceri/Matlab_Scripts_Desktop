function [groundtruth] = get_ddsm_groundtruth_modified(overlay_filename, image)
%
...
if strcmp(all_lines{end},'') 
    all_lines = all_lines(1:end-1);
end

% Find out how many abnormalities there are.
[x1, x2, x3, x4, x5, names] = ... % We ignore the x variables.
...
for(i = 1:num_abnormalities)
  abnormalities{i} = get_abnormality(i, all_lines, image);
end

groundtruth = abnormalities;


function [abnormality] = get_abnormality(abnormality_number, all_lines, image)
%
...
abnormality = parse_abnormality(abnormality, image);


function [abnormality] = parse_abnormality(abnormality_text, image)
%
...
abnormality.annotations = parse_boundary_and_cores(abnormality_text(total_outlines_line_number+1:end), image);
...
function [annotations] = parse_boundary_and_cores(bc_text, image)
%
...
for i=1:length(bc_text)
  ...
  switch code_is
    case 'boundary'
      annotations.boundary = ...
        @(image_dims) make_annotation_image(image_dims, bc_text{i}, image);
    case 'core'
...
function [anno_image] = make_annotation_image(image_dims, chain_code, image)
%
...
% handle cases that index at 0
if(find(pos == 0) > 0)
    index = pos == 0;
    pos(index) = 1;
end

% grab X and Y positions for cropping ROI
listOfXPositions = pos(1);
listOfYPositions = pos(2);
...
for i = 3:length(chain_code)
  ...
  listOfXPositions(end+1) = pos(1);
  listOfYPositions(end+1) = pos(2);
  
  % indexes for rectangular region to crop
  Xmax = max(listOfXPositions);
  Xmin = min(listOfXPositions);
  Ymax = max(listOfYPositions);
  Ymin = min(listOfYPositions);
  
  % Update this pixel.
  anno_image(pos(1), pos(2)) = 1;
  
  % add some padding to better see ROI
  for j=1:10
      if(pos(1) + j < size(anno_image,1) && pos(2) + j < size(anno_image,2))
        anno_image(pos(1)+j, pos(2) + j) = 1;
      end
  end
end

%To view crops with border for checking
imwrite(anno_image(Xmin:Xmax,Ymin:Ymax), 'D:\DDSM\CROP\test.png');
fp = fopen('D:\DDSM\GT\test.txt','wt');
fprintf(fp, '%4.0f %4.0f %4.0f %4.0f', Ymin, Xmin, Ymax, Xmax); %really Xmin Ymin Xmax Ymax
fclose(fp);
